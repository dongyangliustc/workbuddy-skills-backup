...
A Codex/Claude skill for legally downloading, retrying, and reading academic PDFs through the user's own logged-in University of Science and Technology of China (USTC) Library / WebVPN / discovery system / publisher browser session.

中文简介：这是一个面向中科大图书馆/WebVPN 场景的文献下载、失败重试与全文读取 skill。它使用用户自己已经登录的 Chrome 会话，在授权范围内保存 PDF 和 supporting information，并对文件做页数、PDF 签名和文本可读性验证。适合"网页里能打开 PDF，但命令行下载 403/401/登录页""USTC SSO 中断批量下载""ScienceDirect/出版商人机验证后需要继续同一页下载"的情况。

中文快速使用教程：

1. 先在自己的 Chrome 里打开中科大图书馆或 WebVPN，并用自己的账号登录。
   - 常用入口：`https://lib.ustc.edu.cn/`
   - WebVPN：`https://wvpn.ustc.edu.cn/`
   - 文献检索系统（读秀）：`http://www.duxiu.com/`
...
8. 如果 Claude Code 没有自动识别这个 skill，把仓库安装到 `%USERPROFILE%\.claude\skills\ustc-literature-downloader`，然后重启或刷新 Claude Code。
9. 如果遇到中科大统一身份认证 / USTC SSO，不要把账号密码发给 agent。若 Chrome 已经自动填好账号密码，你可以明确授权 agent 只点一次"登录/确认登录"；若出现扫码、短信/OTP、验证码、人机验证或安全提示，则需要你自己在 Chrome 里完成。
10. 如果遇到 ScienceDirect 的 `Are you a robot?` 或其它出版商验证，让 agent 停在当前 tab，自己手动完成验证后再让 agent 从同一个页面继续。不要让 agent 反复刷新、随机点击或并发打开很多页。

可以这样对 agent 说：

```text
请使用 ustc-literature-downloader，通过我已经登录的中科大图书馆/WebVPN Chrome 会话，下载下面这些 DOI 的 PDF 和补充材料，并生成 manifest。
```

## What It Solves

- USTC Library / WebVPN can open a paper, but direct `curl` or `Invoke-WebRequest` returns 403.
- A DOI/title list needs small-batch PDF and supporting information collection.
- USTC SSO interrupts a batch and the failed papers need to be retried after the user manually authenticates in Chrome.
...
Do not paste school account passwords, SSO passwords, SMS codes, OTP codes, QR login results, cookies, or session tokens into chat. The intended workflow is browser handoff: the agent opens the page, the user completes authentication in Chrome, and the agent resumes after the user confirms. If the USTC SSO page is already filled by Chrome's password manager and you explicitly authorize it, the agent may click the visible login/confirm button once without reading or typing credentials.
...
1. Chrome is open.
2. The user has personally logged in to USTC Library / WebVPN / search system in Chrome.
...
This skill is adapted from [baihe26/zju-literature-downloader](https://github.com/baihe26/zju-literature-downloader) for USTC.

With the Skills CLI:

```powershell
npx skills add baihe26/ustc-literature-downloader -g
```

Manual Codex installation:

```powershell
git clone https://github.com/baihe26/ustc-literature-downloader.git "$env:USERPROFILE\.codex\skills\ustc-literature-downloader"
```

Manual Claude Code installation:

```powershell
git clone https://github.com/baihe26/ustc-literature-downloader.git "$env:USERPROFILE\.claude\skills\ustc-literature-downloader"
```

Manual Agents-style installation:

```powershell
git clone https://github.com/baihe26/ustc-literature-downloader.git "$env:USERPROFILE\.agents\skills\ustc-literature-downloader"
```
...
Copy-Item -Recurse -Force `
  "$env:USERPROFILE\.codex\skills\ustc-literature-downloader" `
...
```powershell
pip install -r "$env:USERPROFILE\.codex\skills\ustc-literature-downloader\requirements.txt"
```
...
```text
Use ustc-literature-downloader to download these DOIs through my logged-in USTC WebVPN session, including supporting information, and make a manifest.
```

The skill instructs the agent to:

1. verify Chrome/WebVPN/remote-debugging prerequisites;
2. search the USTC library discovery system by DOI or exact title;
...
- Start from the USTC library `在线全文` links when possible.
...
## SSO / CAS Retry Workflow

Some publishers may still ask for USTC SSO / unified identity authentication after WebVPN is open. This is common with Elsevier/ScienceDirect, Springer Nature, Nature Portfolio, Wiley, Taylor & Francis, Cell Press, and other platforms that use institutional sign-in.
...
1. Ask the agent to collect all SSO-blocked papers into `cas_retry.tsv`.
...
3. Use `cas_waiting_user` for papers currently stopped at USTC SSO.
4. Let the agent open one or a few failed links in Chrome.
5. When the browser reaches SSO, the agent should pause.
6. If Chrome has already filled the USTC SSO account/password and the page only needs a login/confirm click, you can explicitly authorize the agent to click that visible button once.
...
```text
不要让我提供账号密码。请把 SSO 失败文献整理成 cas_retry.tsv，然后逐篇用已登录 Chrome 打开。遇到中科大 SSO 时，如果 Chrome 已经自动填好账号密码，可以在我授权后只点一次登录/确认登录；遇到验证码、人机验证、扫码、短信/OTP、WebVPN 重新登录或二次认证时暂停，让我手动完成认证；认证完成后继续下载 PDF 和 supporting information。URL 格式失败的文献请按 DOI 重新定位出版社页面和真实 PDF 链接。
```

Do not process many SSO tabs at once. Work in small groups so the user can clearly see which page needs authentication and so the institutional session does not get confused.
...
```text
请用 ustc-literature-downloader 小批量下载这些 DOI。ScienceDirect 和其它出版商一次只处理一篇，优先从中科大图书馆/WebVPN/在线全文进入。遇到 Are you a robot、Cloudflare、验证码或出版商人机验证时不要自动点击，也不要反复刷新；请记录到 publisher_verification.tsv，停在当前 tab 让我手动完成，然后从同一个页面继续下载。
```
...
```powershell
$env:PYTHONUTF8='1'
python -X utf8 "$env:USERPROFILE\.claude\skills\ustc-literature-downloader\scripts\extract_pdf_text.py" --pdf "D:\papers\paper.pdf" --pages 3
```

- Library search system URLs often contain `#!` or other fragment-based routing. If that nested URL is passed through another URL without encoding, the fragment can be stripped and Chrome may open `about:blank` or the wrong page. Prefer `scripts/cdp_open_url.mjs` for fragment-heavy URLs.

## Helper Scripts

Open a library search or publisher URL through the CDP proxy without losing `#!` fragments:
...
```powershell
$node = "$env:LOCALAPPDATA\OpenAI\Codex\bin\node.exe"
& $node "$env:USERPROFILE\.claude\skills\ustc-literature-downloader\scripts\browser_pdf_downloader.mjs" `
...
For SSO retries, use the richer template in `examples/cas-retry-template.tsv`.
...
The workflow has been verified with the original zju-literature-downloader test case. When adapting to USTC access, verify with a DOI that your institution can access:
...
- Route: Library search system `PDF` link -> ACS PDF page
